<script lang="ts">
  ...
  function pcodeForFeature(f: any) {
    return f?.properties?.PCODE ?? f?.properties?.pcode ?? f?.properties?.Pcode ?? f?.properties?.ADM_PCODE ?? null;
  }
  ...
  function draw() {
    ...
    console.log('AdminChoropleth: drawing', features.features.length, 'features; dataMap size', Object.keys(dataMap).length);
    // clear
    container.innerHTML = '';
    ...
    try {
      projection.fitExtent([[20, 20], [width - 20, height - 20]], features as any);
    } catch (e) {
    ...
    entries.forEach(([k, v], i) => {
      ...
      legend.append('text').attr('x', 20).attr('y', y + 11).attr('font-size', 12).text(v.label);
    });
  }
