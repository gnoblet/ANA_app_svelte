...
<div class="text-base-content/60 mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 {size}">
	...
	{#each keys as fk (fk)}
		...
		{#if fb}
			<span class="flex items-center gap-1">
				{#if btnCircle}
					<span class="btn btn-circle badge-sm {fb.buttonCls}"></span>
				{:else}
					<span
						class="inline-block h-3 w-3 rounded"
						style="background-color: var({fb.tintVar})"
					></span>
				{/if}
				{fb.label}
			</span>
