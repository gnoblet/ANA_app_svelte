<script lang="ts">
	...
	// ── Color helper ──────────────────────────────────────────────────────────────
	// For leaf indicator nodes: color by flag status from flagRow.
	// For structural nodes (system/factor/subfactor): use hierarchy palette.
	function circleColor(d: d3.HierarchyCircularNode<PackDatum>): string {
		...
		return systemFillColor(systemId);
	}
	...
	onDestroy(() => {
		...
		if (zoomTimer) zoomTimer.stop();
		if (subfactorLabelTimer) clearTimeout(subfactorLabelTimer);
	});
</script>
		...
		{#if root}
			{#each root.descendants() as d (d.data.id)}
				...
				>
					...
					<!-- curved arc labels for depth 1, 2, and 3 -->
