...
import type { IndicatorsRoot } from '../src/lib/types/structure';
...
function loadIndicators(jsonPath: string): IndMeta[] {
	if (!fs.existsSync(jsonPath)) {
		console.error(`Error: indicators.json not found at ${jsonPath}`);
		process.exit(2);
	}
	const root = JSON.parse(fs.readFileSync(jsonPath, 'utf-8')) as IndicatorsRoot;
	const out: IndMeta[] = [];
	for (const sys of root.systems)
		for (const fac of sys.factors)
			for (const sf of fac.sub_factors)
				for (const ind of sf.indicators)
					out.push({ code: ind.indicator, type: ind.type ?? null, preference: ind.preference });
	return out;
}
