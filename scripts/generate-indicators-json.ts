...
function build(rows: RefRow[]): {
	...
	for (const row of rows) {
		...
		metricMap.get(indKey)!.push({
			metric: id,
			metric_description: nullIfEmpty(row['Metric'] ?? ''),
	...
	// Hierarchy: system → factor → subfactor → indicator concept → metric (leaf).
				...
				children: fac.sub_factors.map((sf) => ({
					...
					children: sf.indicators.map((ind) => ({
						...
						children: ind.metrics.map((m) => ({
							name: m.metric_description ?? m.metric,
							id: m.metric,
...
function main() {
	...
	if (help) {
		console.log('Usage: bun generate-indicators-json.ts [--csv <path>] [--out <path>]');
		process.exit(0);
	}
